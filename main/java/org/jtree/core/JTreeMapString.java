...
public class JTreeMapString extends JTreeMap<String> {
	private static final Log logger = LogFactory.getLog(JTreeMapString.class);
	
	public JTreeMapString() {
		super();
    ...
    public void put(String root, String keyNames, HashMap <String,String> newMap) {
        ...
        for (String key:keyNamesArray) {
            ...
            if (mapKey==null) {
                logger.warn("Key for keys " + key + " is null " );
                return;
            }
            ...
            pointer=pointer+super.getDelimiter()+mapKey;
        }
        logger.debug("add " + root + " " + pointer);
        try {
            this.insert(pointer, newMap);
        } catch (Exception e) {
            logger.error("Problem with setting hashmap with size " + newMap.size() + "  to " + pointer, e);  //To change body of catch statement use File | Settings | File Templates.
        }
    }
